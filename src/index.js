...
server.get('/proyectos', async(req, res) => {
    try{
        ...
        const [result] = await connection.query('SELECT * FROM defaultdb.proyectos');
...
server.get('/proyectos/:idAuthor', async (req, res) => {
    ...
    try{
        const connection = await getConnection();
        const [result] = await connection.query('SELECT * FROM defaultdb.proyectos WHERE proyectos.id_author  = ?;', [id]);
        await connection.end();
...
server.post('/subir-proyecto', async (req, res) => {
    ...
    const {projectName, slogan, repo, demo, technologies, description, projectPhoto} = req.body;
   
    try {
    const connection = await getConnection();
    const [resultAuthor] = await connection.query('INSERT INTO defaultdb.authors (authorName, job, authorPhoto) VALUES (?,?,?);', [authorName, job, authorPhoto]);

    const [resultProject] = await connection.query('INSERT INTO defaultdb.proyectos (projectName, slogan, repo, demo, technologies, description, projectPhoto, id_author) VALUES (?,?,?,?,?,?,?,?);', [projectName, slogan, repo, demo, technologies, description, projectPhoto, resultAuthor.insertId]);
    await connection.end();
